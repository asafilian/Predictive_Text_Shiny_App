...
# Introduction {#intro}
The  [*Shiny Predictive Text App*](https://asafilian.shinyapps.io/as_txtpredict)^[https://asafilian.shinyapps.io/as_txtpredict] (Fig. \ref{fig:app-page}) is a web-based application suggesting words the end user may wish to insert in a text field. The current report describes the technical aspect of the product. 
...
Please see [Sect. 4](#model), where we describe why we decided to work on a 50% fraction of the original corpus and implemented a quad-gram model on it. 
...
set.seed(2019)
blogs_ind <- sample(length(blogs), length(blogs) * 0.5)
twitter_ind <- sample(length(twitter), length(twitter) * 0.5)
news_ind <- sample(length(news), length(news) * 0.5)
...
# Models Evaluation Information Table
...
memory <- c("148.03 MB", "147.97 MB", "147.91 MB", 
            "131.50 MB", "131.44 MB", "131.39 MB", 
            "109.23 MB", "109.17 MB", "109.12 MB")
...
As we see in the table, there is no a significant difference between precisions (both top-1 and top-3) of the models. Therefore, considering the memory and file sizes, we choose the 50% fraction for our model.   
